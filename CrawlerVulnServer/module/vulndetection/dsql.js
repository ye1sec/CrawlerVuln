            ...
            for(let key in dbmsError){
                for(let val in  dbmsError[key]){
                    ...
                    if(reg.test(res.text)){
                        let data={url:url,payload:decodeURI("%29%28%22%27"),request:newUrl,resheaders:JSON.stringify(res.headers) ,response:res.text};
                        dbsql.addSQL(data);//添加sql信息
                        //console.log(data);
                        return;
                    }
